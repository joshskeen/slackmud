...
let(:room){ FactoryGirl.create(:room, players: [josh, shopkeeper])}
  ...
  describe 'with a single shopkeeper present' do
    ...
    it "with sufficient funds" do
      ...
      expect(slack_messenger).to receive(:msg_room).with(room.slackid,"josh skeen purchases a chessboard soaked in blood from Lars the Shopkeeper for 10 nerdcoin.\n")
